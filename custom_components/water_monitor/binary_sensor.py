...
from typing import Any, Deque, Optional, Tuple, List
...
async def async_setup_entry(
    ...
    if bool(opts.get(CONF_LOW_FLOW_ENABLE, DEFAULTS[CONF_LOW_FLOW_ENABLE])):
        ...
        if flow_sensor:
            ...
            entities.append(LowFlowLeakBinarySensor(entry=entry, name=f"{prefix} Low-flow leak"))
        else:
    ...
    if bool(opts.get(CONF_TANK_LEAK_ENABLE, DEFAULTS[CONF_TANK_LEAK_ENABLE])):
        prefix = opts.get(CONF_SENSOR_PREFIX) or "Water Monitor"
        entities.append(TankRefillLeakBinarySensor(entry=entry, name=f"{prefix} Tank refill leak"))
...
class TankRefillLeakBinarySensor(BinarySensorEntity):
    ...
    def __init__(self, entry: ConfigEntry, name: str) -> None:
        ...
        ex = {**entry.data, **entry.options}
        self._min_volume = float(ex.get(CONF_TANK_LEAK_MIN_REFILL_VOLUME, DEFAULTS[CONF_TANK_LEAK_MIN_REFILL_VOLUME]))
        self._tol_pct = float(ex.get(CONF_TANK_LEAK_TOLERANCE_PCT, DEFAULTS[CONF_TANK_LEAK_TOLERANCE_PCT]))
        self._repeat = int(ex.get(CONF_TANK_LEAK_REPEAT_COUNT, DEFAULTS[CONF_TANK_LEAK_REPEAT_COUNT]))
        self._window_s = int(ex.get(CONF_TANK_LEAK_WINDOW_S, DEFAULTS[CONF_TANK_LEAK_WINDOW_S]))
        self._clear_idle_s = int(ex.get(CONF_TANK_LEAK_CLEAR_IDLE_S, DEFAULTS[CONF_TANK_LEAK_CLEAR_IDLE_S]))
        self._cooldown_s = int(ex.get(CONF_TANK_LEAK_COOLDOWN_S, DEFAULTS[CONF_TANK_LEAK_COOLDOWN_S]))
    ...
    # Resolve the WaterSessionSensor entity by unique_id and subscribe to its state changes
    ...
    async def _resolve_and_subscribe(self) -> None:
        ...
        entity = next((e for e in ent_reg.entities.values() if e.platform == DOMAIN and e.unique_id == unique_id), None)
        if entity is None:
    ...
    async def _evaluate(self, now: datetime) -> None:
        ...
        if count >= self._repeat:
            ...
            if self._attr_is_on and self._last_event and (now - self._last_event).total_seconds() >= self._clear_idle_s:
                self._attr_is_on = False
